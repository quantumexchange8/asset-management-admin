...
const loadLazyData = (event) => {
    ...
    try {
        setTimeout(async () => {
            ...
            const url = route('broker_accounts.getPendingAccountData', params);
            const response = await fetch(url);
            const results = await response.json();

            accounts.value = results?.data?.data;
            pendingAccountCounts.value = results?.pendingAccountCounts;
            totalPendingCapital.value = results?.totalPendingCapital;
...
watch([pendingAccountCounts, totalPendingCapital], () => {
    emit('update-totals', {
        pendingAccountCounts: pendingAccountCounts.value,
        totalPendingCapital: totalPendingCapital.value,
    });
});
        ...
        <template #content>
            <div class="w-full">
                ...
                >
                    ...
                    <template #empty>
                        <div v-if="pendingAccountCounts === 0">
                            <EmptyData
                        ...
                        >
                            <template #body="{ data }">
                                ...
                                <div class="text-xs text-surface-500 mt-1">
                                    {{ dayjs(data.created_at).subtract(8, 'hour').format('hh:mm:ss A') }}
                                </div>
                            </template>
                        </Column>
