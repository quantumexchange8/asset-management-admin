...
const onSelectedFiles = (event) => {
    form.deposit_history_file = event.target.files[0]; 
};

//useform
const form = useForm({
    deposit_history_file: null,
});
...
const submit = () => {
    form.post(route('transaction.history.importDepositHistory'), {
        onSuccess: () => {
            visible.value = false;
            form.reset();
            toast.add({
        ...
        onError: (errors) => {
            ...
            form.errors.deposit_history_file = str;
        }
    });
};
...
const closeModal = () => {
    ...
    form.reset();
    form.errors.broker = '';
    form.errors.deposit_history_file = '';
}

//download excel tempalte
const downloadTemplate = () => {
    // Directly link to the Laravel route for downloading
    window.location.href = '/transaction/history/download-template';
};
                ...
                <div class="space-y-2">
                    ...
                    >
                        <template #header="{ chooseCallback, clearCallback, files }">
                            ...
                            <InputError :message="form.errors.deposit_history_file" />
                        </template>
