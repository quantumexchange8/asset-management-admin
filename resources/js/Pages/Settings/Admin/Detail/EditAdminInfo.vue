...
const form = useForm({
    ...
    //permission_id: props.admin.permissions.map(permission => permission.id),
...
onMounted(() => {
    selectedRole.value = props.admin?.role || null;
    if (props.admin.permissions) {
        selectedPermissions.value = props.admin.permissions.map(permission => permission.id);
    } else {
...
const submitForm = () => {
    ...
    form.put(route('settings.updateAdminInfo'), {
        onSuccess: () => {
